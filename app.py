...
# Agar foydalanuvchi tizimga kirmagan bo'lsa, faqat login formasini ko'rsatish
if not st.session_state.logged_in:
    ...
    st.stop() # Kodning qolgan qismini to'xtatib turish
...
# Foydalanuvchi roliga qarab tablarni shakllantirish
role = st.session_state.role
tabs_list = ["📈 Boshqaruv Paneli (Dashboard)"]

if role == "admin":
    tabs_list += ["📝 Import Shartnomalari", "🚚 Komplektlar & Logistika", "🏭 Ishlab Chiqarish Liniyasi"]
elif role == "logistics":
    tabs_list += ["🚚 Komplektlar & Logistika"]
elif role == "production":
    tabs_list += ["🏭 Ishlab Chiqarish Liniyasi"]
                ...
                if not df_komplektlar.empty:
                    ...
                    if not kritik_df.empty:
                        for idx, row in kritik_df.iterrows():
                            st.error(f"⚠️ **Konteyner:** {row['Konteyner / Partiya ID']} ({row['Model']})\n\n**Joylashuvi:** {row['Hozirgi Joylashuv']} | **Soni:** {row['Komplektlar Soni']} ta.")
                    else:
                        st.success("Ta'minot zanjiri barqaror.")
                else:
